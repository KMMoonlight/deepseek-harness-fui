...
The durable theme preference, Language choice, and busy-Enter choice follow the DSH user home across reloads, ports, and loopback origins. Product Settings no longer exposes the theme preference because FUI has one palette, but ThemeRuntime still adopts the Host value. Direct edits to `settings.yaml` converge through the existing invalidation stream, while legacy `dsh.theme`, `dsh.locale`, and `dsh.conversation.busyEnter` entries are neither read nor written.
...
Focused unit coverage pins schema registration, listener-before-read ordering, nonblocking activation, schema-validated section acceptance, revisioned ordered writes, stale-response containment, failure recovery, disposal quiescence, and remote memory mode. The namespace-granular scope also carries multi-field sections, so later configuration surfaces can ride the same lifecycle instead of hand-rolling describe/mutate synchronization. The keyless Web settings scenarios write Language and busy-Enter through the UI, verify the YAML document and empty legacy storage, reload, and boot another Host on a distinct port against the same DSH home; theme package and bootstrap tests pin durable theme adoption independently of a product selector.
