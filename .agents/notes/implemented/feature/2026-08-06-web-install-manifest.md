...
This follows code-server's fullscreen choice without copying its `window-controls-overlay` display override. The browser-installed Web application has no custom title bar or safe layout around browser-owned window controls. The [Electron desktop application](../architecture/2026-08-15-electron-fui-desktop-runtime.md) owns its native window integration outside the Web manifest. A manifest override would therefore supersede fullscreen without owning the required browser layout.
