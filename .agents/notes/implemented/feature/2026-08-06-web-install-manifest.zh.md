...
这一选择沿用了 code-server 的全屏方案，但没有照搬其 `window-controls-overlay` 显示覆盖项。浏览器安装的 Web 应用没有自定义标题栏，也没有围绕浏览器自有窗口控件安排安全布局。[Electron 桌面应用](../architecture/2026-08-15-electron-fui-desktop-runtime.md)在 Web manifest 之外负责原生窗口集成。因此，manifest 覆盖项会在未落实浏览器所需布局的情况下取代全屏模式。
