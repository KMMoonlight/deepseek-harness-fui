...
[`THIRD_PARTY_NOTICES.md`](../../../../THIRD_PARTY_NOTICES.md) is generated by [`scripts/gen-third-party-notices.ts`](../../../../scripts/gen-third-party-notices.ts) from the workspace manifests, `vendor/README.md`, the `pyproject.toml` files, and `pnpm-workspace.yaml`. The same generator copies Space Mono's complete copyright and OFL-1.1 text to [`apps/web/public/licenses/space-mono-OFL-1.1.txt`](../../../../apps/web/public/licenses/space-mono-OFL-1.1.txt), which Vite includes in the Web frontend served to browser clients and the desktop shell. The root README pair links the notices file from its License section.
...
The project owner authorizes distribution of the published `@fontsource/space-mono` web-font files under their declared `OFL-1.1` terms. This authorization matches both the exact package identity and exact license declaration; another Fontsource package or a future Space Mono license change fails closed. The Web build preserves the published font bytes and copies the package's copyright notice and complete OFL text to a stable public asset. Browser clients receive that Web distribution directly, while the desktop shell receives the same files from its backend. The font software remains under OFL-1.1; no project license statement relicenses it.
...
Space Mono upgrades remain authorized only while the package declares `OFL-1.1` and carries the reviewed copyright and license markers. Regeneration fails on a license change or missing text, and a Web build copies the generated text beside its public assets so browser clients and the backend-served desktop shell can retrieve the notice required by OFL.
