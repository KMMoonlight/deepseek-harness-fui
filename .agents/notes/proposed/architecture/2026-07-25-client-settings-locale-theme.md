...
**Collaboration doctrine (how every later module joins Settings): feature owners self-register.** The Settings shell is a pure composition surface: it only declares slots and renders the chrome structure — zero copy, no locale dependency, and neither importing nor enumerating any feature; for a feature to appear in Settings, its own plugin registers into the corresponding slot — locale registers the Language row and ui-settings-models registers the Models top-level panel. No separate `ui-settings-*` package is created for "a feature's settings page": the settings surface belongs to the feature package itself. Content that belongs to no single feature (the trigger/title/close chrome copy, the General directory with its skeleton rows, the `settings` dictionary) is owned by `ui-settings-general` — the owner of the ownerless copy, not a feature satellite package. ui-theme intentionally registers no row because the shipped FUI has one palette; the [Appearance-row removal decision](../../implemented/simplification/2026-08-15-remove-appearance-settings-row.md) owns that product choice.
...
| Theme preference | `ui-theme` | No Settings row; the fixed-palette FUI retains ThemeRuntime only for startup, stock-surface semantics, and extension consumers |
...
The first phase localizes only the copy inside the Settings overlay; dictionaries stay close to their owners — the chrome plus the General skeletons live in `ui-settings-general`'s `settings` namespace, and feature-row copy lives in each feature package (`settings.locale`, `settings.models`).
...
**A per-feature `ui-settings-*` satellite package for each section.** It divorces the settings surface from the feature itself: changing preference behavior touches two packages, the package count grows linearly with settings items, and the satellite packages depending back on domain services form an intermediate layer that exists purely for the package split. Under feature-owner self-registration that layer does not exist: preference rows ship with their feature packages, and `ui-settings-general` takes in only the ownerless copy (the chrome and the General skeletons), carrying no feature's settings surface.
...
- Each feature row's store initializes from the getter and is thereafter updated by its own change event with local re-renders; ui-theme exposes no settings row while FUI has one palette.
...
- 中文/English switches and is restored after a refresh; ThemeRuntime still resolves a stored light/dark/system value and responds to system color-scheme changes without exposing a Product Settings selector.
...
The apply order of slot declarations and contributions is not fixed, so every section/item registrant must use `ctx.slots.inject()` rather than a service or local-disposer presence signal. Service events may fire before a row's first render, so both a feature row store's init and the inject attach must align to the current snapshot from the getter. Layout must clean up the global attributes it set on unmount, and ThemeRuntime must remove its matchMedia listener on dispose, so nothing lingers after HMR.
