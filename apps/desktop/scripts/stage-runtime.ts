...
const cliEntry = join(staging, 'node_modules/@deepseek-ai/dsh/lib/bin.js')
const frontendEntry = join(staging, 'node_modules/@deepseek-ai/dsh-web-frontend/dist/index.html')
...
async function main(): Promise<void> {
  ...
  for (const path of [cliEntry, frontendEntry, pnpmEntry]) {
    if (!existsSync(path)) throw new Error(`desktop runtime entry missing after staging: ${path}`)
  }
