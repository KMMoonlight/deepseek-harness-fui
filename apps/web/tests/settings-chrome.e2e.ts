...
// section switching, both close paths), the Language row and busy-state Enter
// preference (both Host-backed), Permission as the persisted default for
// subsequently created sessions, and the absence of the obsolete Appearance
// selector from the single-palette product surface.
...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    // General is active by default; Permission and Language are functional,
    // while the single-palette product exposes no Appearance selector.
    ...
    expect(await dialog.getByRole('button', { name: '浅色' }).count()).toBe(0)
    expect(await dialog.getByRole('button', { name: '深色' }).count()).toBe(0)
    expect(await dialog.getByRole('button', { name: '跟随系统' }).count()).toBe(0)
    ...
    expect(await dialog.getByRole('tab', { name: '安装插件', exact: true }).count()).toBe(0)
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    // The settings-owned copy re-registers localized: dialog title, nav, and
    // the Language selector. The rest of the app's copy is outside this row.
    ...
    expect(await enDialog.getByText('Appearance', { exact: true }).count()).toBe(0)
