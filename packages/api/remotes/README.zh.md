...
当前 Client 组合挂载 Goal Remote 贡献、只读 Host 插件清单贡献（`pluginInventory/list`）、桌面插件安装贡献（`pluginInstaller/add`），以及桌面运行时更新贡献（`runtimeUpdater/describe` 与 `runtimeUpdater/update`）。该组合卸载时，Cordis effect 的所有权机制会撤回所有贡献；`@deepseek-ai/dsh-api-gateway/client` 负责描述符校验、可追踪 namespace Service、直接与作用域方法、调用与取消。Client 入口通过 Cordis 消费共享的 `TypertClientRemote` 接口，不导入具体 Gateway；它只以 type-only 形式重新导出 Gateway Client face 的声明合并，因此消费端经由本外观取到转发事件词汇时，运行时不会多出一条通往 Gateway 实现的边。
