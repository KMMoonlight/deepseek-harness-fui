...
import { apply, inject, LayoutController } from '@deepseek-ai/dsh-client-ui-fui-layout/client'
import { apply as nodeApply } from '@deepseek-ai/dsh-client-ui-fui-layout'
import * as invariant from '@deepseek-ai/dsh-client-ui-fui-layout/invariant'
...
describe('ui-fui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the child declarations', async () => {
    ...
    expect(slots.spec('shell.status')).toEqual({ kind: 'list', scope: 'session' })
  })
...
describe('node half + invariant companion', () => {
  ...
  it('invariant companion registers under the package name', async () => {
    ...
    expect(register).toHaveBeenCalledWith('@deepseek-ai/dsh-client-ui-fui-layout', expect.any(Function))
