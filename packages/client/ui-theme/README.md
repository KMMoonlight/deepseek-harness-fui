...
Theme plugin: ThemeRuntime over the --dsw-* token base stylesheets (static scale + alias semantic layers). The service owns the live theme preference (`light`/`dark`/`system`), resolves `system` through `prefers-color-scheme`, and publishes immutable `ThemeSnapshot`s on the `theme/change` event; it never touches the DOM — ui-layout's presenter applies the resolved snapshot (`html { color-scheme }`, `body[data-ds-dark-theme]`, and inline alias tokens). A loopback browser provides the service immediately with `system`, then loads `ui-theme.preference` in the background; ThemeRuntime consumers can write built-in values through the Host settings API, whose local provider stores them in `$DSH_HOME/settings.yaml` by default. Pushed settings changes and reconnects refetch the preference, rapid writes are serialized in call order with namespace revisions, and a rejected latest write reloads the durable value. Product Settings exposes no Appearance row because the shipped FUI surface has one palette. A remote browser cannot access the privileged settings API, so its preference remains process-local. Third-party registered theme ids remain an in-process extension and do not cross the built-in settings schema; removing one never overwrites the last durable built-in preference. The [Host-backed preferences decision](../../../.agents/notes/implemented/bug-fix/2026-08-06-host-backed-web-preferences.md) owns the persistence boundary, and the [Appearance-row removal decision](../../../.agents/notes/implemented/simplification/2026-08-15-remove-appearance-settings-row.md) owns the product surface.
...
- **No product theme selector** — ThemeRuntime retains light/dark/system for startup, stock-surface semantics, and extension consumers, but Product Settings exposes no Appearance row because the FUI surface has one fixed dark palette.
