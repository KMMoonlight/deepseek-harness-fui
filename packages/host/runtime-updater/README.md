...
Desktop-only Host provider for managed official `@deepseek-ai/dsh` runtime updates. `RuntimeUpdaterGateway` publishes the generated direct Remotes `runtimeUpdater/describe`, `runtimeUpdater/check` (tag query without installing), and `runtimeUpdater/update`. The FUI bundle disables the provider unless `DSH_DESKTOP=1`; the constructor repeats that guard and requires exact DSH and FUI versions, a valid supported-DSH semver range, absolute managed-storage, overlay, and pnpm paths, and an HTTPS or loopback registry.
