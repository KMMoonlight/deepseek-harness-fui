...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // The dynamic-plugin runtime's `cordis/<event>` event names are
  // session/wire-visible runtime identifiers, not package references — the
  // apiproxy remote-events contract and the docs quote them verbatim. The
  // bare `'cordis'` tokens in the same files are the panel's UI locale
  // namespace and catalog keys, equally not package references.
